...
func runPHPActorPropertyReferences(
	...
	runErr := cmd.Run()
	if runErr != nil {
		return nil, fmt.Errorf("run phpactor property references: %w: %s", runErr, strings.TrimSpace(stderr.String()))
	}

	return parsePHPActorReferenceRows(stdout.String())
}

// phpactorConfigExtra serializes adapter-managed Phpactor CLI overrides so helper commands share the same
// index location as the live LSP session.
func phpactorConfigExtra(indexPath string) (string, error) {
	configExtra, err := json.Marshal(map[string]string{phpactorIndexerPathKey: indexPath})
	if err != nil {
		return "", fmt.Errorf("marshal phpactor config extra: %w", err)
	}

	return string(configExtra), nil
}

// buildPHPActorIndex prepares the same managed Phpactor index used by the LSP session before references rely
// on project-wide resolution.
func buildPHPActorIndex(ctx context.Context, cacheRoot, workspaceRoot string) error {
	if err := ensureIndexerPathExists(cacheRoot, workspaceRoot); err != nil {
		return err
	}
	...
	//nolint:gosec // Command is fixed and config-extra comes from an adapter-managed cache path.
	cmd := exec.CommandContext(
		context.WithoutCancel(ctx),
		phpactorServerName,
		"--no-interaction",
	...
	runErr := cmd.Run()
	if runErr != nil {
		return fmt.Errorf("build phpactor index: %w: %s", runErr, strings.TrimSpace(stderr.String()))
	}
